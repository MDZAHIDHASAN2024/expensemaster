...
export default function Income() {
  ...
  const fetchIncomeTypes = async () => {
    try {
      const { data } = await API.get('/categories/income-types');
      setIncomeTypes(data);
    } catch (e) {}
  };
  ...
  const filtered = useMemo(() => {
    ...
    return incomes.filter(
      (i) =>
        i.incomeType?.toLowerCase().includes(q) ||
  ...
  const openEdit = (item) => {
    setEditItem(item);
    setForm({
      date: item.date?.split('T')[0] || currentDate(),
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editItem) {
        await API.put('/income/' + editItem._id, form);
    ...
    <div>
      ...
      >
        ...
        <div className="form-group">
          ...
          <select
            ...
            onChange={(e) =>
              setFilters({ ...filters, incomeType: e.target.value })
            }
          >
      ...
      >
        <div className="search-bar" style={{ flex: 1, minWidth: 200 }}>
          <span>🔍</span>
          <input
            placeholder="Search type, description, remarks..."
            value={search}
            onChange={(e) => {
        ...
        <div
          className="summary-bar income-summary"
          style={{ margin: 0, flex: '0 0 auto', gap: 18 }}
        >
          <span>
            <strong>{filtered.length}</strong> records
          </span>
          <span style={{ borderLeft: '1px solid #c6f6d5', paddingLeft: 14 }}>
            Qty: <strong>{filteredQty.toLocaleString()}</strong>
          </span>
          <span className="total" style={{ color: '#276749' }}>
          ...
          <tbody>
            {loading ? (
              ...
              paginated.map((inc, i) => (
                <tr key={inc._id}>
                  ...
                  <td>
                    <span className="badge badge-green">{inc.incomeType}</span>
                  </td>
            ...
            {paginated.length > 0 && (
              <tr className="total-row income-total-row">
                ...
                <td className="amount-cell" style={{ color: '#276749' }}>
                  ৳
                  {paginated.reduce((s, i) => s + i.amount, 0).toLocaleString()}
                </td>
                <td colSpan={2}></td>
              </tr>
            )}
