...
function Spinner({ text }) {
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:12, padding:'40px 20px' }}>
      ...
      <span style={{ color:'var(--text-light)', fontSize:14 }}>{text || 'Loading...'}</span>
    </div>
  );
}
...
export default function Reports() {
  ...
  React.useEffect(() => { API.get('/categories/types').then(r => setTypes(r.data)).catch(()=>{}); }, []);
  ...
  const loadPreview = async () => {
    ...
    try {
      ...
      setPreview(data.expenses); setPreviewTotal(data.total); setPreviewQty(data.totalQty || 0);
  ...
  const exportFile = async (type) => {
    ...
    try {
      ...
      const response = await fetch(url + '?' + buildParams(), { headers: { Authorization: 'Bearer ' + user.token } });
      if (!response.ok) throw new Error('Export failed');
      const blob = await response.blob();
      const link = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = link;
      a.download = 'expenses_' + Date.now() + (type === 'excel' ? '.xlsx' : '.pdf');
      a.click();
  ...
  return (
    <div>
      <div className="page-header"><h1>📋 Expense Reports</h1></div>

      <div className="card" style={{ marginBottom:20 }}>
        <h3 style={{ marginBottom:16, color:'var(--primary)' }}>🔍 Filter Report</h3>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(160px, 1fr))', gap:14 }}>
          ...
          <div className="form-group"><label>Month</label>
            <select value={filters.month} onChange={e => setFilters({...filters, month:e.target.value})}>
              <option value="">All Months</option>{months.map(m=><option key={m.v} value={m.v}>{m.l}</option>)}
            </select>
          </div>
        ...
        <div style={{ display:'flex', gap:10, marginTop:14, flexWrap:'wrap' }}>
          <button className="btn btn-primary" onClick={loadPreview} disabled={loading}>
            {loading ? <><span className="spinner-sm"/>Loading...</> : '👁️ Preview'}
          </button>
          <button className="btn btn-success" onClick={()=>exportFile('excel')} disabled={!!exporting}>
            {exporting==='excel' ? <><span className="spinner-sm"/>Exporting...</> : '📊 Export Excel'}
          </button>
          <button className="btn" style={{ background:'linear-gradient(135deg,#c53030,#e53e3e)', color:'white' }} onClick={()=>exportFile('pdf')} disabled={!!exporting}>
            {exporting==='pdf' ? <><span className="spinner-sm"/>Exporting...</> : '📄 Export PDF'}
          </button>
        </div>
      </div>

      {loading && <div className="card"><Spinner text="Loading preview..." /></div>}

      {!loading && preview.length > 0 && (
        <>
          <div className="summary-bar" style={{ marginBottom:16, display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
            <span><strong>{preview.length}</strong> records</span>
            <div style={{ display:'flex', gap:20, alignItems:'center' }}>
              <span>Qty Total: <strong style={{ color:'var(--primary)' }}>{previewQty.toLocaleString()}</strong></span>
              <span style={{ fontSize:16, fontWeight:700, color:'var(--primary)' }}>Total: ৳{previewTotal.toLocaleString()}</span>
            </div>
          </div>
          <div className="table-wrapper">
            <table>
              <thead>
                <tr><th>#</th><th>Date</th><th>Month</th><th>Year</th><th>Item Type</th><th>Item Description</th><th>Unit</th><th>Qty</th><th>Amount (৳)</th><th>Remarks</th></tr>
              </thead>
              <tbody>
                {preview.map((e, i) => (
                  <tr key={e._id}>
                ...
                <tr className="total-row">
                  ...
                  <td style={{ fontWeight:700, color:'var(--primary)' }}>{previewQty.toLocaleString()}</td>
                  <td className="amount-cell">৳{previewTotal.toLocaleString()}</td>
                  <td></td>
                </tr>
              </tbody>
            </table>
      ...
      {!loading && preview.length === 0 && (
        <div className="card"><div className="empty-state"><div className="empty-icon">📋</div><p>Set filters and click "Preview" to see data</p></div></div>
      )}
    </div>
  );
}
