    ...
    <div>
      ...
      {!loading && tab === 'monthly' && (
        <div className="card">
          ...
          {monthComp.length === 0 ? <div className="empty-state"><div className="empty-icon">📊</div><p>No data</p></div> : (
            <>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={monthComp}>
                  ...
                  <Tooltip formatter={v=>['৳'+v.toLocaleString(),'Amount']} contentStyle={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:8 }} />
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:10, marginTop:16 }}>
                {monthComp.map((m,i)=>(
                  <div key={i} style={{ background:'var(--bg)', borderRadius:8, padding:'10px 14px', textAlign:'center' }}>
                    ...
                    <div style={{ fontSize:16, fontWeight:700, color:'var(--primary)' }}>৳{m.total.toLocaleString()}</div>
      ...
      {!loading && tab === 'weekly' && (
        <div className="card">
          ...
          {weekly.length === 0 ? <div className="empty-state"><div className="empty-icon">📆</div><p>No data</p></div> : (
            <>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={weekly}>
                  ...
                  <YAxis tick={{ fontSize:11, fill:'var(--text-light)' }} tickFormatter={v=>'৳'+(v/1000).toFixed(0)+'k'} />
                  <Tooltip formatter={v=>['৳'+v.toLocaleString(),'Total']} contentStyle={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:8 }} />
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginTop:16 }}>
                {weekly.map((w,i)=>(
                  <div key={i} style={{ background:'var(--bg)', borderRadius:8, padding:'10px 12px', textAlign:'center' }}>
                    ...
                    <div style={{ fontSize:16, fontWeight:700, color:'var(--primary)' }}>৳{w.total.toLocaleString()}</div>
      ...
      {!loading && tab === 'top' && (
        <div className="card">
          ...
          {topItems.length === 0 ? <div className="empty-state"><div className="empty-icon">🏆</div><p>No data</p></div> : (
            <div>
              {topItems.map((item, i) => {
                ...
                return (
                  <div key={i} style={{ marginBottom:14 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5, flexWrap:'wrap', gap:4 }}>
                      ...
                      <span style={{ fontSize:14, fontWeight:700, color:'var(--primary)' }}>৳{item.total.toLocaleString()} <span style={{ fontSize:11, fontWeight:400, color:'var(--text-light)' }}>({item.count}x)</span></span>
                    </div>
      ...
      {!loading && tab === 'comparison' && (
        <div className="card">
          <div className="card-header"><h3>Category × Month Comparison — {year}</h3></div>
          {compTypes.length === 0 ? <div className="empty-state"><div className="empty-icon">🔄</div><p>No data</p></div> : (
                      ...
                      <tr key={i}>
                        ...
                        {MONTHS_SHORT.map((_,mi)=>(
                          <td key={mi} style={{ textAlign:'right', fontSize:12 }}>
                            {itemComp[type]?.[mi+1] ? '৳'+itemComp[type][mi+1].toLocaleString() : '-'}
                          </td>
                        ))}
                        <td className="amount-cell">৳{rowTotal.toLocaleString()}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        <>
          <div className="stat-grid" style={{ marginBottom:18 }}>
            <div className="stat-card" style={{ borderColor:'#e53e3e' }}>
              <div className="stat-icon" style={{ background:'#fff5f5', fontSize:22 }}>💸</div>
              <div className="stat-info">
                <div className="label">Total Expense</div>
                <div className="value" style={{ color:'#e53e3e' }}>৳{vsData.totalExpense?.toLocaleString()}</div>
              </div>
            </div>
            <div className="stat-card" style={{ borderColor:'#38a169' }}>
              <div className="stat-icon" style={{ background:'#f0fff4', fontSize:22 }}>💵</div>
              <div className="stat-info">
                <div className="label">Total Income</div>
                <div className="value" style={{ color:'#38a169' }}>৳{vsData.totalIncome?.toLocaleString()}</div>
              </div>
            </div>
            <div className="stat-card" style={{ borderColor: vsData.balance >= 0 ? '#38a169' : '#e53e3e' }}>
              <div className="stat-icon" style={{ background: vsData.balance >= 0 ? '#f0fff4' : '#fff5f5', fontSize:22 }}>⚖️</div>
              <div className="stat-info">
                <div className="label">Balance</div>
                <div className="value" style={{ color: vsData.balance >= 0 ? '#38a169' : '#e53e3e' }}>
                  {vsData.balance >= 0 ? '+' : ''}৳{vsData.balance?.toLocaleString()}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="card">
            <div className="card-header"><h3>⚖️ Expense vs Income — {year}</h3></div>
            {vsData.monthly?.length === 0 ? <div className="empty-state"><div className="empty-icon">⚖️</div><p>No data</p></div> : (
              <>
                <ResponsiveContainer width="100%" height={320}>
                  <BarChart data={vsData.monthly} margin={{ top:10, right:20, left:0, bottom:5 }}>
                    ...
                    <YAxis tick={{ fontSize:11, fill:'var(--text-light)' }} tickFormatter={v=>'৳'+(v/1000).toFixed(0)+'k'} />
                    <Tooltip formatter={(v,n)=>['৳'+v.toLocaleString(), n==='expense'?'Expense':'Income']} contentStyle={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:8 }} />
                    <Legend />
                    ...
                    <tbody>
                      {vsData.monthly?.map((row, i) => (
                        <tr key={i}>
                          ...
                          <td style={{ color:'#38a169', fontWeight:600 }}>৳{row.income?.toLocaleString()}</td>
                          <td style={{ color:'#e53e3e', fontWeight:600 }}>৳{row.expense?.toLocaleString()}</td>
